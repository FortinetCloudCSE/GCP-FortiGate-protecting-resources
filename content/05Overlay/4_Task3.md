...
* Open a CLI console in the active FortiGate by clicking on the cursor **>_** icon or using SSH to the public management IP.  Copy the below configurations into your favorite text editor and "set local-gw" to the lb-ip. Once completed, copy and paste thes configurations into the cli console.  The below configuration will add the reuqired phase1 anf phase2 interfaces as well as Firewall policies to allow traffic on the tunnel.
